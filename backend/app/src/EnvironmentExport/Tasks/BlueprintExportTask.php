...
class BlueprintExportTask extends BuildTask {
    ...
    public function run($request) {
        ...
        try {
            // Claim queued BlueprintExportRequest
            BlueprintExportRequest::flush_and_destroy_cache();
            $this->claimQueuedBlueprintExportRequest();
            print_r(BlueprintExportRequest::get()->column('ProcessID'));

            /** @var BlueprintExportRequest $environmentExportRequest */
            $environmentExportRequest = BlueprintExportRequest::get()->find("ProcessID", $this->processID);
            ...
            $this->writeBlueprintExportRequest($environmentExportRequest, $file->ID);
    ...
    /**
     * Writes the BlueprintExportRequest after updating the status and linking the generated export file object
     * @param BlueprintExportRequest $environmentExportRequest
     * @param int $exportFileID
     * @return void
     * @throws ValidationException
     */
    private function writeBlueprintExportRequest(BlueprintExportRequest $environmentExportRequest, int $exportFileID) {
        $environmentExportRequest->BlueprintExportFileID = $exportFileID;
     ...
     * @throws ValidationException
     */
    private function writeExportFile(): BlueprintExportFile {
        $environment = Environment::getEnv('APPLICATION_ENVIRONMENT');
        $file = BlueprintExportFile::create();
    ...
    private function collectDataToExport() {
        ...
        $dataObjectClassesToExport = array_values(ClassInfo::subclassesFor(Blueprint::class, false));
    ...
    /**
     * Claims an BlueprintExportRequest for processing. A claim can only success if there is at least 1 queued request
     * and no other request is currently being processed.
     * @return void
     */
    private function claimQueuedBlueprintExportRequest() {
        try {
            $dbConnection = DB::get_conn();
            $dbConnection->transactionStart();
            $environmentExportRequestBeingProcessed = BlueprintExportRequest::get()->filter(["Status" => "PROCESSING"])->first();
            if ($environmentExportRequestBeingProcessed) {
                Logger::infoLog("An BlueprintExportRequest is still being processed, aborting");
                return;
            }

            DB::prepared_query("
                UPDATE `SurfSharekit_BlueprintExportRequest`
                SET ProcessID = CASE
            ...
            var_dump($this->processID);;
            $dbConnection->transactionEnd();
        } catch(Throwable $e) {
            Logger::infoLog("An error occurred while trying to claim an BlueprintExportRequest: {$e->getMessage()}");
